...
void test("MCP server exposes rewrite and normalization tools end to end", async () => {
  ...
  try {
    ...
    assert.deepEqual(toolNames, ["normalize_augment_output", "prepare_augment_rewrite"]);
    ...
    const prepared = await client.callTool({
      ...
      arguments: {
        ...
        recentConversation: [
          ...
          { role: "assistant", text: "The redirect target is probably being overwritten." },
        ],
      },
    });
...
function resolveServerLaunch(): { rootDir: string; command: string; args: string[] } {
  const currentFile = fileURLToPath(import.meta.url);
  const rootDir = findProjectRoot(path.dirname(currentFile));
  const isCompiled = currentFile.includes(`${path.sep}dist${path.sep}test${path.sep}`);
...
function extractContent(result: unknown): Array<{ type: string; text?: string }> {
