...
import { analyzeDraftIntent, detectTaskIntent, resolveEffectiveRewriteMode } from "../src/intent.js";
import { normalizeAugmentOutput, stripOuterMarkdownFences } from "../src/parser.js";
...
void test("prepared rewrite includes deterministic metadata and execution rules", () => {
  const prepared = prepareRewrite(createPromptContext({
...
void test("prepared rewrite includes recent conversation excerpts when present", () => {
  const prepared = prepareRewrite(createPromptContext({
    recentConversation: [
      { role: "user", text: "We already confirmed the bug happens after login.", tokens: 13 },
      { role: "assistant", text: "The redirect likely comes from the callback handler.", tokens: 12 },
    ],
  }));

  assert.match(prepared.promptSpec, /Recent conversation:/);
  assert.match(prepared.promptSpec, /\[user\] We already confirmed the bug happens after login\./);
  assert.match(prepared.promptSpec, /\[assistant\] The redirect likely comes from the callback handler\./);
  assert.match(prepared.promptSpec, /Use these excerpts only if they materially improve the rewrite\./);
});
...
void test("prepared rewrite keeps plain rewrites out of execution-contract wording", () => {
  ...
  assert.match(prepared.promptSpec, /Rewrite the draft into a stronger prompt/i);
  assert.doesNotMatch(prepared.promptSpec, /smallest strong execution contract/i);
});
...
void test("normalizeAugmentOutput strips leading labels for plain rewrites", () => {
  assert.equal(
    normalizeAugmentOutput("Rewritten prompt: Explain how model routing works in this codebase."),
